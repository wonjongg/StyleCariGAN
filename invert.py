...
class TO_VGG(object): 
    def __init__(self, device="cuda"):
        self.s_mean = torch.from_numpy(np.asarray([0.5, 0.5, 0.5])).view(1,3,1,1).type(torch.FloatTensor).to(device)
        self.s_std = torch.from_numpy(np.asarray([0.5, 0.5, 0.5])).view(1,3,1,1).type(torch.FloatTensor).to(device)
        self.t_mean = torch.from_numpy(np.asarray([0.485, 0.456, 0.406])).view(1,3,1,1).type(torch.FloatTensor).to(device)
        self.t_std = torch.from_numpy(np.asarray([0.229, 0.224, 0.225])).view(1,3,1,1).type(torch.FloatTensor).to(device)
...
def invert(g_ema, perceptual, real_img, device, args):
    ...
    for idx in pbar:
        ...
        loss_feature = []
        for (fake_feat, real_feat) in zip(fake_feature.values(), real_feature.values()):
    ...
    if save:
        with torch.no_grad():
            ...
            utils.save_image(
                sample,
                args.result_dir+f"/{args.image_name}_recon_w_final.png", # should be same as recon_w_final.png
                nrow=int(sample.shape[0] ** 0.5),
                normalize=True,
                range=(-1, 1),
            )
    ...
    wp = w.unsqueeze(1).repeat(1,args.num_layers,1).detach().clone() # single image
    ...
    if save:
        with torch.no_grad():
            sample, _ = g_ema(wp, noise=noises, input_is_w_plus=True, randomize_noise=False) 
            utils.save_image(
                sample,
                args.result_dir+f"/{args.image_name}_recon_wp_initial.png", # should be same as recon_w_final.png
                nrow=int(sample.shape[0] ** 0.5),
                normalize=True,
                range=(-1, 1),
            )
    ...
    for idx in pbar:       
        ...
        fake_img, _ = g_ema(wp_tilde, noise=noises, input_is_w_plus=True, randomize_noise=False)
        fake_img_vgg = to_vgg(fake_img)
    ...
    with torch.no_grad():
        fake_img, _ = g_ema(wp, noise=noises, input_is_w_plus=True, randomize_noise=False)
    if save:
        utils.save_image(
                fake_img,
...
def parse_args():
    ...
    parser.add_argument('--image', type=str, required=True, help='path to the image to invert')
