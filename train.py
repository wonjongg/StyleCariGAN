...
from model import Discriminator, ResNet18, Discriminator_feat
...
def train(args, generator, discriminator_photo, discriminator_cari, discriminator_feat_p,
        discriminator_feat_c, g_optim, d_optim_p, d_optim_c, d_optim_fp, d_optim_fc, g_ema, p_cls, c_cls, id_net, device):
    ...
    for idx in pbar:
        ...
        ret = generator(noise, noise_fine, truncation_latent=mean_latent, mode='p2c')
        fake_feat = ret['co']
        ...
        ret = generator(noise, noise_fine, truncation_latent=mean_latent, mode='c2p')
        fake_feat = ret['po']
        ...
        fake_img = generator(noise, noise_fine, truncation_latent=mean_latent, mode='p2c')['result']
        real_img = generator(noise, noise_fine, truncation_latent=mean_latent, mode='c_gt')
        ...
        fake_img = generator(noise, noise_fine, truncation_latent=mean_latent, mode='c2p')['result']
        real_img = generator(noise, noise_fine, truncation_latent=mean_latent, mode='p_gt')
        real_img = real_img.detach()
        ...
        ret_p2c_recon = generator(noise, truncation_latent=mean_latent, mode='p2c_recon')
        ...
        cyc_id_loss = F.mse_loss(id_net(ret_p2c_recon['result']), id_net(ret_p2c_recon['org']).detach())
        ...
        if get_rank() == 0:
            ...
            if i % 100 == 0:
                with torch.no_grad():
                    g_ema.eval()
                    ret = g_ema([sample_z], truncation_latent=mean_latent, mode='p2c')
                    utils.save_image(
                        ret['result'],
                    ...
                    ret = g_ema([sample_z], truncation_latent=mean_latent, mode='c2p')
                    utils.save_image(
                        ret['result'],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_p", type=str, default='./checkpoint/generator_ffhq.pt')
    parser.add_argument("--ckpt_c", type=str, default='./checkpoint/generator_cari.pt')
    parser.add_argument('--ckpt_pa', type=str, default='./checkpoint/attribute/photo_resnet.pth', help='photo attr classifer checkpoint')
    parser.add_argument('--ckpt_ca', type=str, default='./checkpoint/attribute/cari_resnet.pth', help='cari attr classifier checkpoint')
    ...
    discriminator_photo = Discriminator(
        256, channel_multiplier=args.channel_multiplier
    ).to(device)
    ...
    discriminator_feat_p = Discriminator_feat(
            32, channel_multiplier=args.channel_multiplier
    ).to(device)
    ...
    train(args, generator, discriminator_photo, discriminator_cari, discriminator_feat_p,
            discriminator_feat_c, g_optim, d_optim_p, d_optim_c, d_optim_fp, d_optim_fc, g_ema, p_cls, c_cls, id_net, device)
